...
bool RPCClient::start(quint16 port, QHostAddress ipAddress)
{
	...
	/*TODO: UPDATE COMMON LIB AND UNCOMMENT THIS!*/
	return RpcClientBase::start(port, ipAddress);
	//return RpcClientBase::start(port);
}
...
void RPCClient::rpcSendNiippData(QByteArray data)
{
    ...
    if(mode == 01)
    {
		m_mapController->get_map_client(1)->slot_niipp_power_cicle(id, _zone[zone], ba);
    }

    if(mode == 10)
    {
		m_mapController->get_map_client(1)->slot_update_sector(id, _zone_dir[zone], course, NULL);
    }
