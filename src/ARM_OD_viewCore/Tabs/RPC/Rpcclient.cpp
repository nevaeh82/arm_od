...
//////
	...
	m_dbUavManager->setProperty(0, rec_p);
	...
	m_dbUavManager->setProperty(0, rec_p1);
	...
	m_dbUavManager->setProperty(0, rec_p2);*/
}
...
void RPCClient::rpcSlotServerSendAisData(QByteArray data)
{
	...
	m_mapController->getMapClient(1)->addAis(map);
...
void RPCClient::rpcSlotServerSendAtlantDirection(QByteArray data)
{
	...
	int postId = msg.post.right(1).toInt();

	m_mapController->getMapClient(1)->updatePeleng(msg.sourceId, postId, msg.postLatitude, msg.postLongitude, msg.direction);
}
...
//    qDebug() << "GOT DATA FROM ATLANT!";
	...
	if(!m_mapPelengEnemyIds.contains(msg.sourceId))
	{
		m_mapPelengEnemyIds.insert(msg.sourceId, ++m_pelengEvilIds);
		if(m_pelengEvilIds > 49)
		{
			m_pelengEvilIds = 0;
			m_mapPelengEnemyIds.clear();
		}
	}
	...
	qDebug() << "ID = " << m_mapPelengEnemyIds.size() << m_pelengEvilIds << m_mapPelengEnemyIds.value(msg.sourceId);

	m_mapController->getMapClient(1)->addEnemyBpla( m_mapPelengEnemyIds.value(msg.sourceId), ba1 );
...
void RPCClient::sendBplaPoints(QByteArray data)
{
	...
	m_mapController->getMapClient(1)->addEnemyBpla(m_rdsEvilIds, data);

   /* QMap<QString, QVariant>* rec = new QMap<QString, QVariant>;

	rec = m_dbManagerTarget->getEnemyBplaFields(m_rdsEvilIds);
	if(rec->count() == 0)
	{
	...
	m_dbManagerTarget->setProperty(1, rec_p);
	...
	m_dbManagerTarget->setProperty(1, rec_p1);
