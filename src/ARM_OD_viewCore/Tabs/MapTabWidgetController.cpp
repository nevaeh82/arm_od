...
int MapTabWidgetController::createRPC()
{
	...
	connect(thread_rpc_client, SIGNAL(started()), m_rpcClient, SLOT(slotInit()));

	connect(this, SIGNAL(signalStartRPC()), m_rpcClient, SLOT(slotStart()));
	connect(m_rpcClient, SIGNAL(signalFinished()), thread_rpc_client, SLOT(quit()));
	connect(thread_rpc_client, SIGNAL(finished()), thread_rpc_client, SLOT(deleteLater()));

	connect(m_rpcClient, SIGNAL(signalFinished()), m_rpcClient, SLOT(deleteLater()));
	connect(this, SIGNAL(signalStopRPC()), m_rpcClient, SLOT(slotStop()));
	connect(this, SIGNAL(signalFinishRPC()), m_rpcClient, SLOT(slotFinish()));

	m_rpcClient->setParent(0);
	m_rpcClient->moveToThread(thread_rpc_client);
