	...
	m_rpcClient(NULL)
{
	...
	connect(this, SIGNAL(onMethodCalledInternalSignal(QString,QVariant)), this, SLOT(onMethodCalledInternalSlot(QString,QVariant)), Qt::QueuedConnection);

	m_ktrManager = new TcpKTRManager(this, this);

	m_ktrConnectTimer = new QTimer(this);

	connect(m_ktrConnectTimer, SIGNAL(timeout()), this, SLOT(slotManualKtrConnetion()));
	connect(this, SIGNAL(signalMessageReceived(quint32,QString,MessageSP)), this, SLOT(onMessageReceivedInternal(quint32,QString,MessageSP)), Qt::QueuedConnection);

	m_ktrConnectTimer->start(5000);
}
...
{
	emit signalMessageReceived(deviceType, deviceName, argument);
}
...
void TcpManager::onMessageReceivedInternal(quint32 deviceType, QString deviceName, MessageSP argument)
{
	...
	case DeviceTypes::KTR_TCP_DEVICE:
		if (messageType == TCP_KTR_ANSWER_CONNECTION_STATUS) {
			...
			foreach (const QString &childKey, childKeys)
			{
				...
				{
					boardString = settings.value("boardList", "ololo").toString();
			...
			QStringList boardStringList = boardString.split(",");
			foreach (QString id, boardStringList) {
				boardList.append(id.toInt());
			}
...
void TcpManager::slotManualKtrConnetion()
{
	QList<quint16> boardList;
	...
	BaseTcpDeviceController* controller = m_controllersMap.value("KTR", NULL);
